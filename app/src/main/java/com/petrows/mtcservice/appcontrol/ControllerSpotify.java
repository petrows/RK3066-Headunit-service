...
class ControllerSpotify extends ControllerBase {
  ...
  private final static String TAG = "ControllerSpotify";
  //dsa
  ...
  private boolean isRunning() {
    ...
    for (ActivityManager.RunningServiceInfo pid : am.getRunningServices(200)) {
      if (pid.service.getClassName().contains(SPOTIFY_PACKAGE_NAME)) {
        Log.d(TAG, "Spotify is running: " + pid.service.getClassName());
        return true;
      }
    }

    Log.d(TAG, "spotify is not running");
    return false;
  }
  ...
  private Intent createMediaKeyIntent(long eventtime, int keycode, int action) {
    ...
    intent.setClassName(SPOTIFY_PACKAGE_NAME, SPOTIFY_CLASS_NAME);
